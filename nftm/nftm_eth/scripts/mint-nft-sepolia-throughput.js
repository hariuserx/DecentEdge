...
const mintNFT = async () => {
    ...
    fs.writeFileSync('./results_mint_sepolia_thoughtput_start.txt', startTime.toString() + "\n", { flag: 'a+' }, err => {
        if (err) {
            console.error(err);
        }
    ...
    fs.writeFileSync('./results_mint_sepolia_thoughtput_end.txt', endTime.toString()+ "\n", { flag: 'a+' }, err => {
        if (err) {
            console.error(err);
        }
        // file written successfully
