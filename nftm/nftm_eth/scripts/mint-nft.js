...
const mintNFT = async () => {
    ...
    let nftTxn = await myNftContract.mintNFT(tokenUri, {maxPriorityFeePerGas:  new BigNumber("10000000000").toString(), 
    maxFeePerGas:  new BigNumber("20000000000").toString()})
    ...
    fs.writeFileSync('./results_mint_sepolia_gas_fee.csv', valuesStr, { flag: 'a+' }, err => {
        if (err) {
            console.error(err);
        }
